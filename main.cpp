...
{
	if (rindex < 0 || rindex > 9 || cindex < 0 || cindex>9)
		throw string("The row and column index should be between 0 to 9");
	int newNum;
...
int main()
{
	...
	int rind, cind;
	cout << "Please enter row index to change (0-9): ";
	cin >> rind;
	cout << "Please enter column index to change (0-9): ";
	cin >> cind;
	try
	{
	...
	{
		check = false;
		cout << num << " is a negative and we can't add negative value in array\n";
	}
